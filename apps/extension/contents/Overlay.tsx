...
import { PaperAirplaneIcon, MagnifyingGlassIcon, BookOpenIcon, Square2StackIcon } from "@heroicons/react/24/outline"
import { useState } from "react"
import { useStorage } from "@plasmohq/storage/hook"
import { STORAGE_KEYS, detectSelectionType, getRecommendedLLMs, getRecommendedPrompts, type PromptTemplate, type UserProfile } from "../shared"
...
const Overlay = () => {
  ...
  const [customPrompts] = useStorage<PromptTemplate[]>(STORAGE_KEYS.CUSTOM_PROMPTS, [])
  const [userProfile] = useStorage<UserProfile>(STORAGE_KEYS.USER_PROFILE, "Developer")
  ...
  const handleDirectToss = async () => {
    await handleAction("toss", async () => {
        ...
        // 1. Process Template wrapping
        let finalPrompt = selection
        if (selectedTemplateId) {
          const template = customPrompts?.find(p => p.id === selectedTemplateId)
          if (template) {
        ...
        let targetLLM = settings.preferredLLM || "claude"

        if (userProfile && userProfile !== "Custom") {
           const type = detectSelectionType(selection)
           const recommended = getRecommendedLLMs(userProfile, type)
           if (recommended.length > 0) {
             targetLLM = recommended[0]
           }
        }
        ...
        showToast(`Tossed to ${targetLLM === "claude" ? "Claude" : "ChatGPT"}`, "success")
    })
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* AI Suggestions (Chips) - Phase 3 */}
        {!selectedTemplateId && !loading && (
           <div className="flex gap-1 mr-2 opacity-80 hover:opacity-100 transition-opacity">
              {getRecommendedPrompts(detectSelectionType(window.getSelection()?.toString() || "")).slice(0, 2).map(p => (
                <button
                  key={p.id}
                  onClick={() => {
                    // Quick Toss with Template
                    setSelectedTemplateId(p.id)
                    // We don't auto-toss to allow user to confirm or change LLM, 
                    // but we could auto-toss. For now, just selecting it is safer UX.
                  }}
                  className="px-2 py-0.5 bg-indigo-500/20 hover:bg-indigo-500/40 border border-indigo-500/30 rounded text-[10px] text-indigo-300 hover:text-indigo-100 whitespace-nowrap transition-colors"
                  title={p.prompt}
                >
                  {p.name}
                </button>
        ...
        {/* Template Selector (Phase 3) */}
        {customPrompts && customPrompts.length > 0 && (
          <select
            value={selectedTemplateId}
            onChange={(e) => setSelectedTemplateId(e.target.value)}
            className="w-4 h-4 opacity-50 hover:opacity-100 bg-transparent text-gray-400 text-xs border-none outline-none focus:ring-0 cursor-pointer appearance-none"
